...
if (isset($_POST['submit'])) {
	$id = $_POST['id'];
	$name = $_POST['name'];
	$conn->query("insert into `residingareas` (`AreaName`, `LocalCouncilId`) values ('$name', '$id') ");
	echo ("<script>alert('Saved Successfully');</script>");
	echo ("<script>window.location.href='consumers.php';</script>");
}
?>
			...
			<div class="row">
				...
				<div class="col-md-5">
					<form action="#" class="row" id="contactForm" method="post" enctype="multipart/form-data">
						<h3 class="mt-5">Residing Areas</h3>
						<label>Local Council lD</label>
						<select name="id" class="form-control mb-3" required>
							<?php
							$query = $conn->prepare("SELECT * FROM localcouncils");
							$query->execute();
							$results = $query->fetchAll();
							foreach ($results as $data) {
							?>
								<option value="<?php echo $data['CouncilId']; ?>"><?php echo $data['CouncilName']; ?></option>
							<?php
							}
							?>
						</select>
