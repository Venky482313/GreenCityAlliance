...
if (isset($_POST['submit'])) {
	...
	$price = $_POST['price'];
	$email = $_POST['email'];
	$conn->query("insert into `greenbusinesses` (`BusinessCategoryID`,`CompanyName`,`ContactInformation`,`BusinessDescription`,`BusinessType`,`EnvironmentalBenefits`,`PricingCategory`,`Email`) values ('$bCID','$cName','$contact','$desc','$bType','$env','$price','$email') ");
	echo ("<script>alert('Saved Successfully');</script>");
	echo ("<script>window.location.href='productsservices.php';</script>");
}
?>
		...
		<div class="container">
			...
			<div class="collapse navbar-collapse" id="navbarResponsive">
				<ul class="navbar-nav ms-auto">
					...
					<li class="nav-item mx-0 mx-lg-1">
						<a class="active nav-link py-3 px-0 px-lg-3 rounded" href="greenbusinesses.php">Green Business</a>
					</li>
					<li class="nav-item mx-0 mx-lg-1">
			...
			<div class="row">
				...
				<div class="col-md-5">
					<form action="#" class="row" id="contactForm" method="post" enctype="multipart/form-data">
						<h3 class="mt-5">Green Business</h3>

						<label>Business Category ID</label>
						<select name="bCID" class="form-control mb-3" required>
							<?php
							$query = $conn->prepare("SELECT * FROM businesscategories");
							$query->execute();
						...
						<input type="text" name="cName" class="form-control mb-3" required />
						<label>Contact Information</label>
						<input type="text" name="contact" class="form-control mb-3" required />
						<label>Business Description</label>
						<input type="text" name="desc" class="form-control mb-3" required />
						<label>Business Type</label>
						<select name="bType" class="form-control mb-3" required>
						...
						<select name="price" class="form-control mb-3" required>
